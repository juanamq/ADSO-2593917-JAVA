	...
	.then( data => {
		...
		for (var i = 0; i < data.registros.length; i++) {
			let trHtlm = `	<tr>
								...
								<td class="m-0 p-1">
									<button class="col-12 btn btn-outline-danger m-0 px-2 py-1" onclick="abrirModalEliminar(${i})" > ELIMINAR </button>
								</td>
							</tr>`;
...
function abrirModalEliminar(posicion){
	document.getElementById("campo_eliminar_cedula").value = listaClientes[posicion].cedula;
	document.getElementById("campo_eliminar_nombres").value = listaClientes[posicion].nombres;
	document.getElementById("campo_eliminar_apellidos").value = listaClientes[posicion].apellidos;
	document.getElementById("campo_eliminar_telefono").value = listaClientes[posicion].telefono;
	document.getElementById("campo_eliminar_direccion").value = listaClientes[posicion].direccion;
	document.getElementById("campo_eliminar_email").value = listaClientes[posicion].email;
